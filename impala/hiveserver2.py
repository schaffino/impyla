...
class HiveServer2Connection(Connection):
    ...
    def cursor(self, user=None, configuration=None, convert_types=False,
               batch_cursor=False, dictify=False, fetch_error=True):
        ...
        convert_types : bool, optional
            ...
            expensive.) Only applies to HS2 protocol versions <6
        ...
        if batch_cursor:
            cursor_class = HiveServer2BatchCursor
        elif dictify:
            cursor_class = HiveServer2DictCursor
        else:
            cursor_class = HiveServer2Cursor
...
class HiveServer2BatchCursor(HiveServer2Cursor):
    """cursor that returns a batch of rows rather than individual rows""" 
    def __next__(self):
        if not self._last_operation.is_columnar:
            raise NotSupportedError("Server does not support columnar "
                                    "fetching")
